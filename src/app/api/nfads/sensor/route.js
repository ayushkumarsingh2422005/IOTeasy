...
export async function GET(request) {
  try {
    ...
    if (startDate && endDate) {
      ...
      const firstRecord = await Nfads.findOne().sort({ createdAt: 1 }).lean();
      const lastRecord = await Nfads.findOne().sort({ createdAt: -1 }).lean();
    ...
    const records = await Nfads.find(queryFilter).sort({ createdAt: 1 }).lean();
    ...
    console.error('Error in NFADS sensor route:', err);
